...
var ReadingStrip = class {
    ...
    constructor(settings, indicator) {
        ...
        //     visible: true
        // });
        // stripCoverWidget.set_position(196, 149);
        // stripCoverWidget.height =  20;
    ...
    toggleReadingStrip(indicator) {
		...
		this._dbusConnection = this._subscribeSignal('WindowPicked', (conn, sender, obj_path, iface, signal, windowIdVariant) => {
            ...
            if (targetMetaWindow) {
                log(targetMetaWindow.get_title())              
                ...
                const strip_h = new St.Widget({
                    reactive: true,
                    can_focus: true,
                    track_hover: true,
                    visible: true
                });
                strip_h.set_position(targetMetaWindowRect.x, targetMetaWindowRect.y);
                strip_h.height =  targetMetaWindowRect.height * 0.2;
                strip_h.style = 'background-color : rgb(246,211,45)';
                strip_h.visible = true;
                strip_h.opacity = 40;
