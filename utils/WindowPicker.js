...
var WindowPickerServiceProvider = class WindowPickerServiceProvider {
  ...
  constructor() {
    const iFace = ByteArray.toString(
      Me.dir.get_child('dbus-interfaces').get_child('org.gnome.Shell.Extensions.stripcover.PickWindow.xml').load_contents(null)[1]);
  ...
  PickWindow() {
    ...
    const inspector = new StripcoverWindowPickerInspector(Main.createLookingGlass());
    ...
    inspector.connect('target', (me, target, x, y) => {
      ...
      if (actor.toString().includes('WindowActor')) {
          ...
          variant = new GLib.Variant('(s)', [windowId]);
      } else {
  ...
  enable() {
    this._dbus.export(Gio.DBus.session, '/org/gnome/shell/extensions/stripcover');
  }
...
const StripcoverWindowPickerInspector = GObject.registerClass({
  GTypeName: 'StripcoverWindowPickerInspector',
  Signals: {
    'WindowPickCancelled': {}
  }
}, class StripcoverWindowPickerInspector extends LookingGlass.Inspector {
  _init(lookingGlass) {
